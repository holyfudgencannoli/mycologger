...
export default function CreateRecipeBatch({ setUnsaved }: { setUnsaved: (value: boolean) => void }) {
    ...
    const handleSubmit = async () => {
        ...
        for (let i = 0; i < ingredients.length; i++) {
            ...
            if (!inventoryLog) {
                const newInventoryLog = await InvLog.create(db, 'raw_material', RM.id, 0, ingredient.unit, created_at)
            }
            ...
            const use = await InvLog.update(
                db,
                'raw_material',
                newInventoryLog? newInven : inventoryLog.id,
                null,
                baseUsage,
