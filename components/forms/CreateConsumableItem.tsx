...
import * as ConItem from './../../data/db/consumable-items'
...
export default function CreateConsumableItem() {
    ...
    const handleSubmit = async () => {
        const type = 'consumable_items'
        const created_at = new Date().getTime()
        try {
            const invItemId = await InvItem.create(db, type, created_at);
            console.log(invItemId)
            const ConItemId = await ConItem.create(db, invItemId, name, category, subcategory)
            console.log(ConItemId)
            const invLogId = await InvLog.create(db, type, ConItemId, 0, '', new Date().getTime())
            navigation.navigate("Consumable Items List")
            return { invItemId, ConItemId }
        } catch (error) {
            console.error(error)
        }
    }
    ...
    return(
            <Surface style={styles.surfaceMetaContainer}>                        
                <Surface style={styles.surfaceContainer}>
                        <Text style={theme.formTitle}>New Consumable Item</Text>        
